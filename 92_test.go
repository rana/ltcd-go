...
func reverseBetween(hed *ListNode92, lft, rht int) *ListNode92 {
...
func TestReverseBetween(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			hed:  createList92([]int{1, 2, 3, 4, 5}),
			lft:  2,
			rht:  4,
			want: []int{1, 4, 3, 2, 5},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := reverseBetween(tt.hed, tt.lft, tt.rht)
			gotVals := listToSlice92(got)
			if !sliceEqual92(gotVals, tt.want) {
...
func createList92(vals []int) *ListNode92 {
	if len(vals) == 0 {
		return nil
	}
...
func listToSlice92(hed *ListNode92) []int {
	var res []int
	cur := hed
	for cur != nil {
...
func sliceEqual92(a, b []int) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
